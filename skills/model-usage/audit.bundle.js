...
var APPLY = has("--apply");
var JSON_OUT = has("--json");
var BY_PROJECT = has("--by-project");
var BY_REPO = has("--by-repo");
var BY_ACCOUNT = has("--by-account");
var daysArg = argv.find((a) => a.startsWith("--days="));
var DAYS = daysArg ? Number(daysArg.split("=")[1]) : null;
var HOME = homedir();
...
var AssistantRecord = exports_external.object({
  type: exports_external.literal("assistant"),
  timestamp: exports_external.string().optional(),
  message: exports_external.object({ model: exports_external.string(), usage: Usage })
});
var SettingsPatch = exports_external.object({
  model: exports_external.string().optional(),
  effortLevel: exports_external.enum(["low", "medium", "high", "xhigh"]).optional(),
  autoCompactEnabled: exports_external.boolean().optional(),
  skillListingMaxDescChars: exports_external.number().optional(),
  fastModePerSessionOptIn: exports_external.boolean().optional(),
...
function repoFromCwd(cwd) {
  ...
  let repo;
  const m = base.match(/\/wt\/worktrees\/([^/]+?)(?:\.git)?(?:\/|$)/);
  if (m)
    repo = m[1];
  else if (existsSync(base) && gitTop(base))
    repo = gitTop(base);
  else
    repo = "(unknown)";
...
var shortCwd = (cwd) => (cwd.startsWith(HOME) ? "~" + cwd.slice(HOME.length) : cwd).replace(/^~\/\.local\/state\/wt\/worktrees\//, "wt:");
...
`)) {
    ...
    try {
      ...
      if (o.session_id && o.account)
        accountMap.set(o.session_id, o.account);
    } catch {}
  }
} catch {}
var tally = new Map;
...
var byRepo = new Map;
var byAccount = new Map;
var cutoff = DAYS ? Date.now() - DAYS * 86400000 : null;
var addGrp = (map, key, out, cost, opus, newSession) => {
...
var rows = [...tally.entries()].sort((a, b) => b[1].cost - a[1].cost);
var totalCost = rows.reduce((s, [, r]) => s + r.cost, 0) || 1;
var totalOut = rows.reduce((s, [, r]) => s + r.out, 0);
var opusCost = rows.filter(([m]) => m.includes("opus")).reduce((s, [, r]) => s + r.cost, 0);
var opusSharePct = 100 * opusCost / totalCost;
var rollup = (map, label) => [...map.entries()].map(([k, v]) => ({ key: k, label, ...v, opusPct: v.out ? 100 * v.opusOut / v.out : 0 })).sort((a, b) => b.cost - a.cost);
var projects = rollup(byProject, "project").map((p) => ({ ...p, display: p.key.startsWith("(dir:") ? p.key : shortCwd(p.key) }));
var repos = rollup(byRepo, "repo");
var accounts = rollup(byAccount, "account");
var current = {};
try {
  current = JSON.parse(await readFile(SETTINGS, "utf8"));
} catch {}
var recommend = {};
if (opusSharePct > 50 && current.model !== "sonnet")
  recommend.model = "sonnet";
if (current.effortLevel === undefined)
  recommend.effortLevel = "medium";
if (current.autoCompactEnabled === undefined)
  recommend.autoCompactEnabled = true;
if (current.skillListingMaxDescChars === undefined)
  recommend.skillListingMaxDescChars = 256;
if (current.fastModePerSessionOptIn === undefined)
  recommend.fastModePerSessionOptIn = true;
var curEnv = current.env ?? {};
var envWant = {
  CLAUDE_CODE_SUBAGENT_MODEL: "claude-haiku-4-5-20251001",
  CLAUDE_AUTOCOMPACT_PCT_OVERRIDE: "70"
};
var envPatch = {};
for (const [k, v] of Object.entries(envWant))
  if (curEnv[k] !== v)
    envPatch[k] = v;
if (Object.keys(envPatch).length)
  recommend.env = envPatch;
var patch = SettingsPatch.parse(recommend);
var hasPatch = Object.keys(patch).length > 0;
var slim = (arr, keyName) => arr.slice(0, 25).map((p) => ({
  [keyName]: p.display ?? p.key,
  sessions: p.sessions,
  turns: p.turns,
  outM: +(p.out / 1e6).toFixed(2),
  estSpend: +p.cost.toFixed(2),
  spendPct: +(100 * p.cost / totalCost).toFixed(1),
  opusPct: +p.opusPct.toFixed(1)
}));
if (JSON_OUT) {
  console.log(JSON.stringify({
    ...
    models: Object.fromEntries(rows.map(([m, r]) => [m, {
      inM: +(r.in / 1e6).toFixed(2),
      outM: +(r.out / 1e6).toFixed(2),
      cacheReadM: +(r.cr / 1e6).toFixed(2),
      estSpend: +r.cost.toFixed(2),
      spendPct: +(100 * r.cost / totalCost).toFixed(1),
      turns: r.n
    }])),
  ...
  const pad = (s, n) => s.length > n ? "\u2026" + s.slice(-(n - 1)) : s.padEnd(n);
  const num = (x, n) => x.toFixed(2).padStart(n);
  const int = (x, n) => String(x).padStart(n);
  const table = (title, arr, keyW, keyOf, limit = 15) => {
    console.log(title);
...
`);
  if (BY_ACCOUNT)
    table(`By account \u2014 ${accounts.length} (joined from ${accountMap.size} stamped sessions):`, accounts, 36, (p) => p.key);
  if (BY_REPO)
    table(`By repo \u2014 ${repos.length} repos (worktrees collapsed via cwd), top 15:`, repos, 28, (p) => p.key);
  if (BY_PROJECT)
    table(`By project \u2014 top 15 of ${projects.length} (cwd):`, projects, 46, (p) => p.display);
  if (!hasPatch) {
