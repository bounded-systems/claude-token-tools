...
let
  ...
  # Absolute path to Claude Code's config dir. home.file keys are relative to
  # $HOME, so they use cfg.configDir directly; activation scripts and launchd
  # need it absolute.
  claudeDir = "${config.home.homeDirectory}/${cfg.configDir}";
  ...
  mergeScript = pkgs.writeShellScript "claude-token-tools-merge-settings" ''
    ...
    f="$1"
    tmp=$(mktemp "''${f}.XXXXXX")
    # Merge token-saving keys into <configDir>/settings.json.
...
in {
  options.programs.claude-token-tools = {
    ...
    configDir = lib.mkOption {
      ...
      description = ''
        Claude Code's config directory, relative to $HOME.

        Claude Code reads $CLAUDE_CONFIG_DIR when it is set, falling back to
        ~/.claude. If you set CLAUDE_CONFIG_DIR, set this to match — otherwise
        this module writes its settings, hooks, and skills to a directory Claude
        Code never reads, and every token-saving key here is silently inert.
      '';
    };
    ...
    model = lib.mkOption {
      type = lib.types.str;
      default = "sonnet";
      description = "Default Claude model written into <configDir>/settings.json.";
    };
    ...
    capVerboseBash = {
      enable = lib.mkOption {
        type = lib.types.bool;
        default = true;
        description = "Deploy the PreToolUse Bash output-cap hook to <configDir>/hooks/.";
      };
    };
    ...
    stampAccount = {
      enable = lib.mkOption {
        type = lib.types.bool;
        default = true;
        description = "Deploy the SessionStart account-stamp hook to <configDir>/hooks/.";
      };
    };
  ...
  config = lib.mkIf cfg.enable {
    ...
    # Deploy hook and skill files to cfg.configDir.
    ...
    home.file = lib.mkMerge [
      (lib.mkIf cfg.capVerboseBash.enable {
        "${cfg.configDir}/hooks/cap-verbose-bash.sh" = {
      ...
      (lib.mkIf cfg.weeklyAudit.enable {
        "${cfg.configDir}/scripts/model-usage-audit.sh" = {
          source = ../scripts/model-usage-audit.sh;
          executable = true;
        };
      })
      {
        "${cfg.configDir}/skills/model-usage/audit.ts".source      = ../skills/model-usage/audit.ts;
        "${cfg.configDir}/skills/model-usage/audit.bundle.js".source = ../skills/model-usage/audit.bundle.js;
        "${cfg.configDir}/skills/model-usage/package.json".source  = ../skills/model-usage/package.json;
        "${cfg.configDir}/skills/model-usage/SKILL.md".source      = ../skills/model-usage/SKILL.md;
      }
    ];
    ...
    launchd.agents.claude-model-usage-audit = lib.mkIf cfg.weeklyAudit.enable {
      ...
      config = {
        ...
        StandardOutPath = "${claudeDir}/model-usage-audit.out";
        StandardErrorPath = "${claudeDir}/model-usage-audit.err";
      };
    };
    ...
    home.activation.claudeTokenToolsCleanup =
      lib.hm.dag.entryBefore [ "checkLinkTargets" ] ''
        for f in \
          ...
          "${claudeDir}/skills/model-usage/audit.bundle.js" \
          "${claudeDir}/skills/model-usage/package.json" \
          "${claudeDir}/skills/model-usage/SKILL.md"; do
        ...
        $DRY_RUN_CMD rm -rf "${claudeDir}/skills/model-usage/node_modules"
      '';

    # Merge token-saving settings into <configDir>/settings.json (additive — does
    # not overwrite hooks, theme, plugins, or permissions already present).
    home.activation.claudeTokenToolsSettings =
      lib.hm.dag.entryAfter [ "writeBoundary" ] ''
        _settingsFile="${claudeDir}/settings.json"
